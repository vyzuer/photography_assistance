...
def run_grid_search(X, Y):
    ...
    n_samples = X.shape[0]
    cv = cross_validation.ShuffleSplit(n_samples, n_iter=3, test_size=0.1, random_state=777)
...
def read_features(db_path, dump_dir, comp=True, view=True, geo=True, time=False):
...
def learn_composition(db, dump_dir, grid_search=False):
    f_result = db + "/results/res_comp." + str(time.time())
    f_name = db + "/.comp.params"
