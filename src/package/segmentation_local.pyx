...
def merge_segments_wrap(double[:,::1] mean_color, Py_ssize_t[:, :] slic_map):
    cdef Py_ssize_t num_segments = np.amax(slic_map)+1

    edge_matrix = merge_edges(mean_color, num_segments)
    seg_map = merge_segments(slic_map, num_segments, edge_matrix)
        
    return seg_map


def merge_edges(mean_color, num_segments):
    ...
    for i in range(num_segments):
        for j in range(i, num_segments):
            if edge_matrix[i][j] == 0:
                dist_color = 0.0
                for c in range(3):
                    dist_color += (mean_color[i][c] - mean_color[j][c]) ** 2
...
def merge_segments(slic_map, num_segments, edge_matrix):
