...
def testing(w_dir, image_dir, dump_path, file_list='image.list', gen_all=False, clean=True):
    ...
    feature_file = dump_path + "composition.fv"
    comp_file = dump_path + "composition_map.fv"
...
if os.access(dump_path, os.R_OK) == False:
    os.mkdir(dump_path)

testing(w_dir, db_src, dump_path, gen_all = True, clean = True)
