...
class SuperPixelSegmentation:
    def __init__(self, image, seg_type=1, min_dist=0.50, compactness=10.0, n_segments=50, sigma=1.0, num_of_salient_objects=30):
        ...
        self.slic_map, segments = slic(self.image, compactness=int(compactness), n_segments=n_segments, sigma=sigma, max_iter=10)
    ...
    def __segmentation(self, segments):
        ...
        if self.seg_type == 1:
            self.mean_color = self.__find_mean_color(segments)
            ...
            if optimize == True:
                seg_map = segmentation_local.merge_segments_wrap(self.mean_color, self.slic_map.astype(np.intp))
            else:
    ...
    def __find_mean_color(self, segments):
        print "starting mean color..."
        timer = time.time()
        height, width, dim = self.image.shape
