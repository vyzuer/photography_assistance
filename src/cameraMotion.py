...
def process_image_list(w_dir, image_dir, dump_path, model_path, seg_dir, file_list='image.list'):
    ...
    for image_name in fp_image_list:
        ...
        pred, t_form, tilt, pan, zoom, rotate = findSuggestion(infile, dump_path, model_path, geo_list[i], seg_dir)
        
        fp_pred.write("{0}\n".format(image_name))
        fp_pred.write('Score :\t%.2f\n' %(pred[0][0]))
        # fp_pred.write('TILT : %.4f\tPAN : %.4f\tZOOM : %.4f\tROTATE : %.4f\n' %(tilt, pan, zoom, rotate))
        fp_pred.write('TILT : %.4f\tPAN : %.4f\tZOOM : %.4f\n' %(tilt, pan, zoom))
