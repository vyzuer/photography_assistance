...
def find_eigen_rules(data, w_dir):
    pca_dump_dir = w_dir + "pca/"
    if not os.path.exists(pca_dump_dir):
        os.makedirs(pca_dump_dir)

    pca_dump = pca_dump_dir + "pca.pkl"
    pca_scalar_dump = pca_dump_dir + "pca_scalar.pkl"

    # pca_scalar = preprocessing.StandardScaler()
    # pca_scalar = preprocessing.MinMaxScaler(feature_range=(0, 1))
    # X = pca_scalar.fit_transform(data)
    
    # X = preprocessing.normalize(data, norm='l2')

    pca = perform_pca(data, w_dir)

    joblib.dump(pca, pca_dump)
    # joblib.dump(pca_scalar, pca_scalar_dump)


def find_base_rules(data, w_dir):
    nmf_dump_dir = w_dir + "nmf/"
    if not os.path.exists(nmf_dump_dir):
        os.makedirs(nmf_dump_dir)

    nmf_dump = nmf_dump_dir + "nmf24.pkl"
    # nmf_scalar_dump = nmf_dump_dir + "nmf_scalar.pkl"

    # nmf_scalar = preprocessing.MinMaxScaler(feature_range=(0, 1))
    # X = nmf_scalar.fit_transform(data)
    # 
    # X = preprocessing.normalize(X, norm='l2')

    nmf = perform_nmf(data, w_dir)

    joblib.dump(nmf, nmf_dump)
    # joblib.dump(nmf_scalar, nmf_scalar_dump)


def main(w_dir):

    comp_list = w_dir + "feature_dumps/comp_map_clean.list"

    print "Reading composition vectors..."
    # data, X, Y = read_composition(w_dir)
    X = np.loadtxt(comp_list)

    # find_eigen_rules(X, w_dir)

    find_base_rules(X, w_dir)
...
def perform_pca(X, w_dir):

    print "Performing PCA..."
    pca = PCA(n_components=48)
    e_rules = pca.fit_transform(X)
    print e_rules[0]
    eigen_components = pca.components_
    # eigen_components = pca.components_[0:40]
    print np.sum(pca.explained_variance_ratio_[0:48])

    # f_name = w_dir + "variance.png"
    # plot_variance(pca.explained_variance_ratio_, f_name)

    print "done."

    f_name = w_dir + "eigen_rules_48.png"
    visualize_eigen_rules(eigen_components, 48, f_name)
...
def perform_nmf(X, w_dir):
    ...
    n_com = 48
    model = ProjectedGradientNMF(n_components=n_com, sparseness='data', beta=1, eta=0.9, tol=0.000001, max_iter=2000, nls_max_iter=5000, random_state=None)
    model.fit(X) 
...
def read_composition(w_dir):
    comp_list = w_dir + "feature_dumps/comp_map_clean.list"
    # comp_list = dump_dir + "comp_map.fv"
    f_score = w_dir + "aesthetic.scores"

    data = np.loadtxt(comp_list)
...
def visualize_base_rules(V, num_comp, f_name):
    # visualize first 24 base rules
    n_rows = num_comp/8
    for i in range(num_comp):
        base_rule = V[i]
        base_rule = np.reshape(base_rule, (DIM_X, DIM_Y)) 
        pyplot.subplot(6,8,i+1)
        pyplot.axis('off')
        pyplot.imshow(base_rule, origin="upper")
...
def visualize_eigen_rules(V, num_rules, f_name):
    # visualize first 10 eigen rules
    ...
    # mean rule
    # eigen_rule = np.reshape(mean, (DIM_X, DIM_Y)) 
    # pyplot.subplot(3,5,num_rules)
    # pyplot.axis('off')
    # pyplot.imshow(eigen_rule, origin="upper")
    pyplot.savefig(f_name)
    pyplot.close()
...
if __name__ == "__main__":
    ...
    # w_dir = "/home/yogesh/Project/Flickr-YsR/esplanade/dump_ImageDB_640_27_June/"
    # w_dir = "/home/yogesh/Project/Flickr-YsR/floatMarina/dump_ImageDB_640_27_June/"
    composition_list = "comp_map.fv"
    if len(sys.argv) != 2:
    ...
    # composition_list = "composition_map.fv"
