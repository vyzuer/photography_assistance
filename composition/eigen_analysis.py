...
def find_eigen_rules(data, w_dir, model_path):
    ...
    perform_pca(data, w_dir, pca_dump)
...
def main(w_dir, model_path):
    ...
    find_base_rules(X, w_dir, model_path)
...
def perform_pca(X, w_dir, pca_dump):

    pca = joblib.load(pca_dump)
    e_rules = pca.transform(X)

    visualize_eigen_rules(e_rules, w_dir)
...
def visualize_base_rules(V, num_comp, f_name):
    ...
    for image_name in fp_image_list:
        ...
        pyplot.axis('off')
        pyplot.imshow(base_rule, cmap=pyplot.cm.Blues, interpolation='none')
        pyplot.savefig(f_name)
        pyplot.close()
...
def visualize_eigen_rules(V, w_dir):
    dump_dir = w_dir + "/eigen_rules_map/"
    if not os.path.exists(dump_dir):
        os.makedirs(dump_dir)
    ...
    for image_name in fp_image_list:
        ...
        eigen_rule = V[i]
        eigen_rule = np.reshape(eigen_rule, (RULE_X, RULE_Y)) 
        pyplot.subplot(1,1,1)
        pyplot.axis('off')
        eigen_rule = np.absolute(eigen_rule)
        pyplot.imshow(eigen_rule, cmap=pyplot.cm.Blues, interpolation='none')
        pyplot.savefig(f_name)
        pyplot.close()
