...
def learn_camera_parameters(db, dump_dir, grid_search=False):
    ...
    f_result = results_dir + "/res_ev." + str(time.time())

    params_dir = db + "params/"
    if not os.path.exists(params_dir):
        os.makedirs(params_dir)

    f_name = params_dir + "/.ev.params"

    model_dir = db + "models/"
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    comp_model_dir = model_dir + "ev/"
    if not os.path.exists(comp_model_dir):
        os.makedirs(comp_model_dir)

    dump_f_scalar = comp_model_dir + "/f_scalar_ev.pkl"
    dump_t_scalar = comp_model_dir + "/t_scalar_ev.pkl"
    dump_pca = comp_model_dir + "/pca_ev.pkl"
    dump_svr = comp_model_dir + "/svr_ev.pkl"
    ...
    if grid_search == False:
        ...
        # C, gamma, epsilon = 32, 8, 0.01
        # C, gamma, epsilon = 0.5, 2, 0.0001
    else:
