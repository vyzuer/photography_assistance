...
if len(sys.argv) < 4:
    sys.exit('Usage: %s dump_path fv.list target_file' % sys.argv[0])
...
def scale_data(data, target):
    ...
    np.savetxt("fv_ap_1.list", X, fmt='%0.6f')
    ...
    np.savetxt("fv_ap_2.list", X, fmt='%0.6f')
    ...
    np.savetxt("cam_scale_ap.list", Y, fmt='%0.6f')
...
def learn_aperture(f_file, v_file):
    # load features
...
learn_aperture(feature_file, target_file)
