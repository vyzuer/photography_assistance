...
def run_grid_search(X, Y):
    ...
    # gamma_range = 2. ** np.array([-2, 0, 2, 5])
    # # epsilon_range = 2. ** np.array([-15, -13, -11 ])
    # epsilon_range= [0.0001, 0.005, 0.01, 0.1]

    C_range = [0.001, 0.01, 0.1, 1, 1.5, 2]
    gamma_range = [0.1, 0.5, 1, 1.5, 2, 2.5, 3]
    epsilon_range= [0.000001, 0.00001, 0.0001, 0.01, 0.1, 1]
...
def read_features(db_path, dump_dir, env=True, view=False, geo=False):
    ...
    ap_value = np.loadtxt(f_target)[:,1:2]
    X = np.hstack([X, ap_value])

    target = np.loadtxt(f_target)[:,0:1]
...
def learn_camera_parameters(db, dump_dir, grid_search=False):
    ...
    f_result = results_dir + "/res_ss." + str(time.time())

    params_dir = db + "params/"
    if not os.path.exists(params_dir):
        os.makedirs(params_dir)

    f_name = params_dir + "/.ss.params"

    model_dir = db + "models/"
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)

    comp_model_dir = model_dir + "ss/"
    if not os.path.exists(comp_model_dir):
        os.makedirs(comp_model_dir)

    dump_f_scalar = comp_model_dir + "/f_scalar_ss.pkl"
    dump_t_scalar = comp_model_dir + "/t_scalar_ss.pkl"
    dump_pca = comp_model_dir + "/pca_ss.pkl"
    dump_svr = comp_model_dir + "/svr_ss.pkl"
